...
public class ParseAndCompareBenchmarkResults {
    ...
    /**
     * Parse the ion-java performance comparison report by the provided keyword. This function is used to provided information
     * requested from the GitHub workflow.
     * @param optionsMap is the hash map which generated by the command line parser which match the option name and its value appropriately.
     * @throws Exception if errors occur when reading the input file.
     */
    public static void parseReport(Map<String, Object> optionsMap) throws Exception {
        String filepath = optionsMap.get("--reportPath").toString();
        String keyword = optionsMap.get("--keyword").toString();
        try (IonReader reader = IonReaderBuilder.standard().build(new BufferedInputStream(new FileInputStream(filepath)))) {
            reader.next();
            IonSystem system = IonSystemBuilder.standard().build();
            IonStruct reportStruct = (IonStruct) system.newValue(reader);
            switch (keyword) {
                case "input":
                    System.out.println(reportStruct.get("input").toString().replace("\"", ""));
                    break;
                case "params":
                    System.out.println(reportStruct.get("params").toString());
                    break;
                case "·gc.alloc.rate":
                    System.out.println(((IonStruct)reportStruct.get("relative_difference_score")).get("·gc.alloc.rate")
                            .toString());
                    break;
                case "Heap usage":
                    System.out.println(((IonStruct)reportStruct.get("relative_difference_score")).get("Heap usage")
                            .toString());
                    break;
                case "Serialized size":
                    System.out.println(((IonStruct)reportStruct.get("relative_difference_score")).get("Serialized size")
                            .toString());
                    break;
                case "speed":
                    System.out.println(((IonStruct)reportStruct.get("relative_difference_score")).get("speed")
                            .toString());
                    break;
            }
        }
